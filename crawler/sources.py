...
# ===================== 全局过滤词（应用于所有来源） =====================
# 正向：链接文本 / 详情页正文 命中任一即保留（与各来源 keywords 叠加使用）
POSITIVE_EXTRA = ["考试", "公务员招考", "公务员考试"]

# 负向：链接文本 / 详情页正文 命中任一即丢弃（覆盖搬迁/关闭/成绩/列表汇总页等噪声）
NEGATIVE_KW = ["搬迁", "关闭", "成绩", "公示公告", "通知公告", "公告列表", "查看更多", "更多信息"]

# 发布日期时间窗（早于此日期的条目不入库，已入库的下次 merge 时会被删除）
MIN_PUBLISH_DATE = "2026-08-01"

# 增量窗口：仅抓取最近 N 天内（按发布日期）新增内容
RECENT_DAYS = 2
